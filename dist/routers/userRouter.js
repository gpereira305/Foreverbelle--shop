...
const userRouter = _express.default.Router(); // autenticação do admin
...
})); // autenticação de usuário se já é registrado
...
})); // autenticação de usuário ao se registrar

userRouter.post("/signup", (0, _expressAsyncHandler.default)(async (req, res) => {
  const user = new _userModel.default({
    name: req.body.name,
    email: req.body.email,
    password: req.body.password
